...
describe('subscriptionItemFeaturesRouteConfigs', () => {
  ...
  describe('Route config completeness', () => {
    ...
    it('should handle route config array merging correctly', () => {
      // This test verifies that the base route configs (generated by
      // generateOpenApiMetas) are properly concatenated with the custom
      // expire route config using array spread syntax.
      ...
      // Verify the export is an array (as expected by restRoutes aggregation)
      expect(
        Array.isArray(subscriptionItemFeaturesRouteConfigs)
      ).toBe(true)
      expect(
        subscriptionItemFeaturesRouteConfigs.length
      ).toBeGreaterThan(0)
    })
  })
})
