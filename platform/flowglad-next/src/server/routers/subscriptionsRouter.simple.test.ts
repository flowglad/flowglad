...
import { setupOrg, setupUserAndApiKey, setupUserAndCustomer, setupPaymentMethod, setupSubscription, setupBillingPeriod, setupSubscriptionItem } from '@/../seedDatabase'
import { subscriptionsRouter } from './subscriptionsRouter'
import { SubscriptionAdjustmentTiming, SubscriptionItemType, SubscriptionStatus, BillingPeriodStatus } from '@/types'
...
describe('Simple Subscriptions Router Test', () => {
  test('should call adjust endpoint successfully', async () => {
    ...
    const billingPeriod = await setupBillingPeriod({
      ...
      endDate: subscription.currentBillingPeriodEnd || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
