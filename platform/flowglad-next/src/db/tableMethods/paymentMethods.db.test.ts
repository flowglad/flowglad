  ...
  it('Scenario 4: No payments for the organization in the specified date range', async () => {
    ;(
      await adminTransactionWithResult(async ({ transaction }) => {
        ...
        expect(resultDate.getUTCDate()).toBe(
          expectedDate.getUTCDate()
        )
        expect(
          resultDate.toISOString().startsWith('2023-05-01T')
        ).toBe(true)
      ...
      ;(
        await adminTransactionWithResult(async ({ transaction }) => {
          ...
          await setupPayment({
            ...
            invoiceId: invoiceJuly.id,
            chargeDate: new Date(
              '2023-07-03T10:00:00.000Z'
            ).getTime(), // Falls within toDate
          })
          ...
          await setupPayment({
            ...
            invoiceId: invoiceJuly.id,
            chargeDate: new Date(
              '2023-07-10T14:00:00.000Z'
            ).getTime(), // Falls outside toDate, but in same month interval
          })
    ...
    it('should work with customerId filters', async () => {
      ;(
        await adminTransactionWithResult(async ({ transaction }) => {
          ...
          expect(result.items.length).toBe(1)
          expect(result.items[0].payment.id).toBe(payment1.id)
          expect(result.items[0].payment.customerId).toBe(
            customer1.id
          )
          expect(result.total).toBe(1)
    ...
    it('should handle multiple payments with same customer name', async () => {
      ;(
        await adminTransactionWithResult(async ({ transaction }) => {
          ...
          const paymentIds = result.items.map(
