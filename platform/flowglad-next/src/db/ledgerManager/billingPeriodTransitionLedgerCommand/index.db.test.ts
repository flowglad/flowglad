        ...
        ;(
          await adminTransactionWithResult(
            async ({ transaction }) => {
              ...
              expect(creditEntry.ledgerAccountId).toBe(
                ledgerAccount.id
              )
        ...
        ;(
          await adminTransactionWithResult(
            async ({ transaction }) => {
              ...
              expect(typeof onceEntry.sourceUsageCreditId).toBe(
                'string'
              )
              const onceCredit = (
                await selectUsageCreditById(
          ...
          await adminTransactionWithResult(
            async ({ transaction }) => {
              ...
              createdAccounts.forEach((account) => {
                expect(account.subscriptionId).toBe(
                  testSubscription.id
                )
        ...
        ;(
          await adminTransactionWithResult(
            async ({ transaction }) => {
              ...
              expect(nonRenewingCommand.payload.type).toBe(
                'non_renewing'
              )
              ...
              expect(
                (nonRenewingCommand.payload as any)
                  .previousBillingPeriod
              ).toBeUndefined()
        ...
        ;(
          await adminTransactionWithResult(
            async ({ transaction }) => {
              ...
              const initialCreditIds =
                initialResult.ledgerEntries.map(
                  (e) =>
              ...
              const renewingBillingPeriod = await setupBillingPeriod({
                ...
                startDate:
                  updatedSubscription.currentBillingPeriodStart!,
              ...
              expect(renewingResult.ledgerEntries[0].amount).toBe(
                1000
              ) // Recurring amount
        ...
        ;(
          await adminTransactionWithResult(
            async ({ transaction }) => {
              ...
              expect(initialCredit.status).toBe(
                UsageCreditStatus.Posted
              )
              return Result.ok(undefined)
            }
