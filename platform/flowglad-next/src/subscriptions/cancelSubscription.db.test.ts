...
describe('Subscription Cancellation Test Suite', async () => {
    ...
    it('falls back to the organization default pricing model when the customer lacks one', async () => {
      ...
      ;(
        await adminTransaction(async (ctx) => {
          ...
          expect(defaultSubscriptions).toHaveLength(1)
          expect(defaultSubscriptions[0].priceId).toBe(
            defaultPrice.id
          )
          return Result.ok(undefined)
        })
    ...
    it('should only abort scheduled billing runs and not affect other statuses', async () => {
      ;(
        await adminTransaction(async (ctx) => {
          ...
          expect(updatedFailedRun.status).toBe(
    ...
    it('handles immediate timing by canceling future billing periods and aborting scheduled runs', async () => {
      ;(
        await adminTransaction(async (ctx) => {
          ...
          expect(
            updatedSubscription.cancelScheduledAt
          ).toBeGreaterThan(0)
    ...
    it('returns scheduled cancellations without events for non-immediate timing', async () => {
      ;(
        await adminTransaction(async (ctx) => {
          ...
          expect(
            response.unwrap().subscription.cancelScheduledAt
          ).toBe(now + 60 * 60 * 1000)
          return Result.ok(undefined)
        })
      ).unwrap()
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should return an error for invalid subscription input', async () => {
      ...
      // The function throws when accessing subscription.customerId on null,
      // which adminTransaction converts to a Result.err
      const result = await adminTransaction(async (ctx) => {
        const { transaction } = ctx
        const innerResult = await cancelSubscriptionImmediately(
          {
            subscription: null as unknown as Subscription.Record,
          },
          createDiscardingEffectsContext(transaction)
        )
        expect(Result.isError(innerResult)).toBe(true)
        return Result.ok(undefined)
      })
      // The transaction should fail because cancelSubscriptionImmediately throws
    ...
    it('should handle subscription with runBillingAtPeriodStart = true', async () => {
      ;(
        await adminTransaction(async (ctx) => {
          ...
          expect(scheduledRuns.length).toBe(1)
          expect(scheduledRuns[0].scheduledFor).toBe(
            futureBP.startDate
          )
          return Result.ok(undefined)
        })
