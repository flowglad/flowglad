  ...
  Result.gen(async function* () {
      ...
      if (freeSubscriptions.length > 0) {
        ...
        if (params.preserveBillingCycleAnchor) {
          ...
          params.preservedBillingPeriodStart =
            subscriptionToCancel.currentBillingPeriodStart ||
            undefined
    ...
    if (hasTrialPeriod) {
      ...
      if (Result.isError(priceResult)) {
        return Result.err(new NotFoundError('Price', params.price.id))
      }
    ...
    const { subscription, subscriptionItems } = yield* Result.await(
      insertSubscriptionAndItems(params, transaction)
    )
    ...
    if (notificationDecision.sendCustomerNotification) {
      if (
        notificationDecision.customerNotificationType ===
          'upgraded' &&
