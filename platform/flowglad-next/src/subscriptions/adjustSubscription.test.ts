...
describe('adjustSubscription Integration Tests', async () => {
  ...
  describe('Adjustment Notifications', () => {
    ...
    it('should include correct adjustmentId for idempotency', async () => {
      const item1 = await setupSubscriptionItem({
        subscriptionId: subscription.id,
        name: 'Premium Plan',
        quantity: 1,
      ...
      await adminTransaction(async ({ transaction }) => {
        ...
        // Verify both notifications have the same adjustmentId for idempotency
        const customerAdjustmentId =
          mockCustomerNotification.mock.calls[0][0].adjustmentId
        const orgAdjustmentId =
          mockOrgNotification.mock.calls[0][0].adjustmentId
