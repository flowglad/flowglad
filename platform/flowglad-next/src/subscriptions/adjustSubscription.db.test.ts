    ...
    it('should trigger billing run with correct params when upgrading (adding items, increasing quantity)', async () => {
      ...
      ;(
        await comprehensiveAdminTransactionWithResult(async (ctx) => {
          ...
          const mockTrigger = getMockTrigger()
          expect(mockTrigger).toHaveBeenCalledTimes(1)
          const triggerCall = mockTrigger.mock
            .calls[0][0] as any as any
          expect(
    ...
    it('should send downgrade notifications when rawNetCharge is zero or negative (downgrade)', async () => {
      ...
      ;(
        await comprehensiveAdminTransactionWithResult(async (ctx) => {
          ...
          const mockCustomerNotification =
            getMockCustomerNotification()
          const mockOrgNotification = getMockOrgNotification()
          ...
          expect(customerPayload.previousItems[0].unitPrice).toBe(
            4999
          )
          ...
          const orgPayload = mockOrgNotification.mock
            .calls[0][0] as any
        ...
        ;(
          await comprehensiveAdminTransactionWithResult(
            async (ctx) => {
              ...
              if (result.status === 'ok') {
                expect(
                  result.value.pendingBillingRunId
                ).toBeUndefined()
              }
        ...
        ;(
          await comprehensiveAdminTransactionWithResult(
            async (ctx) => {
              ...
              expect(
                allClaims.map((c) => c.externalId).sort()
              ).toEqual(['user-1', 'user-2', 'user-3', 'user-4'])
