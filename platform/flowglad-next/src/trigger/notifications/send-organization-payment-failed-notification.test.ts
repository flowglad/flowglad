...
/**
 * Simulates the payment-failed notification trigger task logic.
 * This mirrors the actual trigger implementation to test the filtering behavior.
 */
const simulateSendPaymentFailedNotification = async (
...
describe('send-organization-payment-failed-notification', () => {
  ...
  describe('recipient filtering for paymentFailed notifications', () => {
    it('sends email only to users with paymentFailed=true and testModeNotifications=true for testmode events', async () => {
      ...
      const result = await simulateSendPaymentFailedNotification(
        organizationId,
        false // testmode
      )
    ...
    it('sends email to all users with paymentFailed=true for livemode events regardless of testModeNotifications', async () => {
      await createUserWithPreferences({
        organizationId,
    ...
    it('does not call safeSend when all users have paymentFailed disabled', async () => {
      await createUserWithPreferences({
        organizationId,
        email: 'user@test.com',
        notificationPreferences: {
    ...
    it('uses default preferences (paymentFailed=true) for users with empty preferences in livemode', async () => {
      await createUserWithPreferences({
        organizationId,
