...
/**
 * Simulates the subscription-canceled notification trigger task logic.
 */
const simulateSendSubscriptionCanceledNotification = async (
...
describe('send-organization-subscription-canceled-notification', () => {
  ...
  describe('recipient filtering for subscriptionCanceled notifications', () => {
    it('sends email only to users with subscriptionCanceled=true and testModeNotifications=true for testmode events', async () => {
      ...
      const result =
        await simulateSendSubscriptionCanceledNotification(
          organizationId,
          false
        )
    ...
    it('sends email to all users with subscriptionCanceled=true for livemode events regardless of testModeNotifications', async () => {
      await createUserWithPreferences({
        organizationId,
    ...
    it('does not call safeSend when all users have subscriptionCanceled disabled', async () => {
      await createUserWithPreferences({
        organizationId,
        email: 'user@test.com',
        notificationPreferences: {
    ...
    it('uses default preferences (subscriptionCanceled=true) for users with empty preferences in livemode', async () => {
      await createUserWithPreferences({
        organizationId,
