...
import { z } from 'zod'
import { CacheDependency, cached, getTtlForNamespace } from './cache'
import { invalidateDependencies } from './cache.internal'
...
describe('dependency-based invalidation (Redis-backed)', () => {
  ...
  it('deletes dependency registry Set after invalidation', async () => {
    ...
    // The registry Set is deleted after invalidation
    expect(mockRedis.sets[registryKey]).toBeUndefined()
  })
})

describe('invalidateDependencies (simplified)', () => {
  let mockRedis: ReturnType<typeof createMockRedisClient>
  ...
  it('deletes cache entries for dependencies', async () => {
    ...
    const cachedFn = cached(
      {
        ...
        keyFn: () => 'delete-test',
        schema: testSchema,
        dependenciesFn: (_result) => ['dep:delete-test'],
      },
      async () => ({ data: 'cached-value' })
    )

    // Populate cache
    await cachedFn()

    const cacheKey = `${RedisKeyNamespace.SubscriptionsByCustomer}:delete-test`
    ...
    await invalidateDependencies(['dep:delete-test'])

    // Cache entry should be deleted
    expect(mockRedis.store[cacheKey]).toBeUndefined()
  })

  it('does not attempt recomputation after invalidation', async () => {
    // This test verifies that invalidateDependencies simply deletes cache entries
    // without any recomputation logic (which was removed)
    ...
    const cachedFn = cached(
      {
        namespace: RedisKeyNamespace.ItemsBySubscription,
        keyFn: () => 'no-recompute-test',
        schema: testSchema,
        dependenciesFn: (_result) => ['dep:no-recompute'],
      },
      async () => {
        callCount.value++
        return { count: callCount.value }
      }
    )

    // Populate cache (first call)
    await cachedFn()
    expect(callCount.value).toBe(1)

    // Invalidate the dependency
    await invalidateDependencies(['dep:no-recompute'])

    // Wait a bit to ensure no async recomputation is triggered
    await new Promise((resolve) => setTimeout(resolve, 50))

    // The wrapped function should NOT have been called again
    // (no automatic recomputation)
    expect(callCount.value).toBe(1)

    // Cache should be empty
    const cacheKey = `${RedisKeyNamespace.ItemsBySubscription}:no-recompute-test`
    expect(mockRedis.store[cacheKey]).toBeUndefined()
  })
})
