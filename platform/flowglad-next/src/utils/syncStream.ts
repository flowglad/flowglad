...
/**
 * Poll for new events after lastSequence.
 * Used by SSE connection loop. Returns immediately with any new events
 * (non-blocking since Upstash REST API doesn't support XREAD BLOCK).
 */
export const pollSyncEvents = async (params: {
  ...
  try {
    ...
    // XREAD returns null/empty if no new entries
    ...
    // XREAD returns an array of [streamKey, entries[]]
    const streamData = result[0]
    if (!streamData || streamData.length < 2) {
      return []
    }

    const entries = streamData[1] as Array<
    ...
    logger.error('Failed to poll sync events', {
      error: errorMessage,
      key,
      lastSequence,
    })
