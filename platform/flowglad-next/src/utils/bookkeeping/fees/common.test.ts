...
import { PriceType, PaymentMethodType, DiscountAmountType, CountryCode, CurrencyCode, FeeCalculationType, StripeConnectContractType } from '@/types'
import {
...
import { setupOrg, setupCustomer, setupPayment, setupInvoice } from '@/../seedDatabase'
import { adminTransaction } from '@/db/adminTransaction'
import { insertFeeCalculation } from '@/db/tableMethods/feeCalculationMethods'
import { BillingAddress, Organization } from '@/db/schema/organizations'
...
describe('calculatePriceBaseAmount', () => {
  it('returns price unit price when no purchase exists', () => {
    ...
    expect(calculatePriceBaseAmount({ price, purchase: null })).toBe(1000)
  })
  it('returns firstInvoiceValue for single payment purchases', () => {
    const price = { unitPrice: 1000 } as any
    const purchase = { priceType: PriceType.SinglePayment, firstInvoiceValue: 800 } as any
    expect(calculatePriceBaseAmount({ price, purchase })).toBe(800)
  })
  it('returns pricePerBillingCycle for subscription purchases', () => {
    const price = { unitPrice: 1000 } as any
    const purchase = { priceType: PriceType.Subscription, pricePerBillingCycle: 900 } as any
    expect(calculatePriceBaseAmount({ price, purchase })).toBe(900)
  })
})
...
describe('calculateDiscountAmount', () => {
  ...
  it('returns fixed amount for fixed discounts', () => {
    const discount = { amountType: DiscountAmountType.Fixed, amount: 500 } as any
    expect(calculateDiscountAmount(1000, discount)).toBe(500)
  })
  it('calculates percentage discount correctly', () => {
    const discount = { amountType: DiscountAmountType.Percent, amount: 20 } as any
...
describe('calculateInternationalFeePercentage', () => {
  const org = { feePercentage: '1.0', stripeConnectContractType: 'Platform' } as any
...
describe('calculatePaymentMethodFeeAmount', () => {
  it('calculates card fee correctly', () => {
    expect(calculatePaymentMethodFeeAmount(1000, PaymentMethodType.Card)).toBe(59)
  })
  it('caps bank account fee', () => {
    expect(calculatePaymentMethodFeeAmount(100000, PaymentMethodType.USBankAccount)).toBe(500)
  })
})
...
describe('calculateTotalFeeAmount & calculateTotalDueAmount', () => {
  ...
  it('calculates total due correctly', () => {
    const dueCalc = { baseAmount: 1000, discountAmountFixed: 100, taxAmountFixed: 90 } as any
