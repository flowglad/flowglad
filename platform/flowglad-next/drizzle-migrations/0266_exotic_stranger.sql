-- Add 'resource' value to FeatureType enum for resource-based features
...
END $$;
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "features_resource_id_idx" ON "features" USING btree ("resource_id");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "subscription_item_features_resource_id_idx" ON "subscription_item_features" USING btree ("resource_id");--> statement-breakpoint
...
CREATE POLICY "Enable read for customers (resources)" ON "resources" AS PERMISSIVE FOR SELECT TO "customer" USING ("organization_id" = current_organization_id() and "active" = true and "pricing_model_id" in (select "pricing_model_id" from "customers"));--> statement-breakpoint
