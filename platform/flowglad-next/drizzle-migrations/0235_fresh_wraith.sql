...
CREATE POLICY "Enable read for customers (discounts)" ON "discounts" AS PERMISSIVE FOR SELECT TO "customer" USING ("organization_id" = current_organization_id() and "active" = true);--> statement-breakpoint
CREATE POLICY "Enable read for customers (features)" ON "features" AS PERMISSIVE FOR SELECT TO "customer" USING ("organization_id" = current_organization_id() and "active" = true);--> statement-breakpoint
...
CREATE POLICY "Enable read for customers (products)" ON "products" AS PERMISSIVE FOR SELECT TO "customer" USING ("organization_id" = current_organization_id() and "active" = true and "pricing_model_id" in (select "pricing_model_id" from "customers"));--> statement-breakpoint
...
CREATE POLICY "Enable read for customers (usage_meters)" ON "usage_meters" AS PERMISSIVE FOR SELECT TO "customer" USING ("pricing_model_id" in (select "pricing_model_id" from "customers"));--> statement-breakpoint
