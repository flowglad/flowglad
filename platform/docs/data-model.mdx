...
Features describe what your customer gets, in terms of `boolean` access to functionality, or a `usage_credit_grant` that grants access to a usage meter. For usage credit grant features, you can specify whether the grant is one time, granted immediately at the time of purchase or subscription creation. Or whether the grant recurs every billing period.

Features are standalone resources in the Flowglad data model. Products can have many features, and features can belong to many products simultaneously. The product <> feature relationship is how Flowglad determines which features to grant customers when they purchase.
