...
Usage prices are optional when creating a usage event. You can create usage events by referencing the usage meter directly (via `usageMeterSlug` or `usageMeterId`) without requiring a price. This is useful for tracking usage without billing. Events created without a price will have a `unitPrice` of $0 and won't generate charges. 
...
Usage events record a consumption of an event to a usage meter. Every usage event belongs to a usage meter and a subscription. Usage events may be associated with a usage price for billing purposes. When usage events are created, they **deduct** from the usage balance on the subscription.
