...
You'll use the `FlowgladServer.createUsageEvent` method. You can create usage events in two ways: with a specific price (using `priceSlug` or `priceId`) or with a usage meter directly (using `usageMeterSlug` or `usageMeterId`). In both cases, the resulting usage event will always have a `priceId`—when using a meter identifier, the system resolves to the meter's default price.
...
You can also create usage events by referencing the usage meter directly using `usageMeterSlug` or `usageMeterId`. When you do this, the system resolves to the meter's **default price**. Every usage meter has a guaranteed default price—either a price you've configured as the default, or an auto-generated [no-charge price](#no-charge-prices) at $0:
...
*   **Identifier (exactly one required):**
    *   `priceSlug` or `priceId`: Reference a specific usage price directly.
    *   `usageMeterSlug` or `usageMeterId`: Reference a usage meter. The system resolves to the meter's default price (which could be a user-configured default or the auto-generated [no-charge price](#no-charge-prices) at $0).
...
Every usage meter has an auto-generated "no-charge" price with the slug pattern `{usagemeterslug}_no_charge`. This price:

- Has `unitPrice: 0` (always free)
- Acts as the default price when no other price is set as default
- Cannot be archived or deleted (protected)
- Has an immutable slug and `unitPrice` (only `name` can be edited)

When you create usage events using a meter identifier (`usageMeterSlug` or `usageMeterId`), the system automatically uses the meter's default price. If no custom default is set, the no-charge price is used.

This ensures every usage event has a `priceId`, simplifying billing calculations and reporting.
