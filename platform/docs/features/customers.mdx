...
Customers are automatically created when someone completes a [checkout session](/features/checkout-sessions). The customer's `externalId` is automatically set based on the authenticated user.

```typescript
// Server-side: Creates a checkout session for the authenticated user
...
Create customers directly when you need to set up customer records before checkout:

```typescript
const result = await flowglad(userId).createCustomer({
  customer: {
    externalId: 'user_123',
...
<Note>
When using `FlowgladServer` with a scoped `customerExternalId`, customers are automatically created via `findOrCreateCustomer()` when needed. Direct customer creation is typically only needed for admin operations.
</Note>
