...
|------------|-------------|---------|
| `id` | Flowglad-generated UUID | `cust_a1b2c3d4...` |
...
Customers are automatically created when someone completes a checkout session. If you provide a `customerExternalId` when creating the checkout session, it will be used as the customer's `externalId`.

```typescript
const { checkoutSession } = await server.createCheckoutSession({
  priceId: 'price_abc123',
  customerExternalId: 'user_123', // Links to your user
...
Create customers directly using the API when you need to set up customer records before checkout:
...
Store custom data on customer records using the `metadata` field. This is useful for syncing additional information from your system:
...
const customer = await flowglad.customers.create({
  ...
  metadata: {
    plan: 'enterprise',
    accountManager: 'john@company.com',
    signupSource: 'webinar',
  },
})
```

Metadata is returned with customer records and can be updated at any time.
...
1. **All active subscriptions are canceled** - Any subscriptions in `active`, `trialing`, `past_due`, or `cancellation_scheduled` status are immediately canceled
...
```bash
curl -X POST https://api.flowglad.com/api/v1/customers/user_123/archive \
...
```typescript
await flowglad.customers.archive({
  externalId: 'user_123',
})
```
...
```typescript
// Good: Use your system's user ID
const customer = await flowglad.customers.create({
  externalId: user.id, // e.g., 'user_123'
  email: user.email,
  name: user.name,
})

// Avoid: Letting Flowglad generate the only identifier
const customer = await flowglad.customers.create({
  email: user.email,
  name: user.name,
})
