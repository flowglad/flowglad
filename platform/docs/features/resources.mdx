...
<Note>
Resources differ from [Usage](/features/usage) in a key way: usage tracks *consumption* that gets billed, while resources track *allocations* that can be reserved and released. A seat can be claimed and later released when someone leaves the team; a consumed API token is gone forever.
</Note>
...
```typescript lib/team-management.ts
export async function getTeamMembers(customerExternalId: string) {
  const server = flowglad(customerExternalId)
...
export async function getTeamCapacity(orgId: string) {
  ...
  return {
    ...
    percentUsed: Math.round((usage.claimed / usage.capacity) * 100)
  }
}
...
export async function addMember(
  ...
  const result = await server.claimResource({
    resourceSlug: 'seats',
    externalId: userId,
    metadata: { email: userEmail, addedAt: new Date().toISOString() }
  })
