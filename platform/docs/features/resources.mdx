...
**On Downgrade:** Downgrade behavior depends on the timing:

- **`timing: 'auto'` (default):** Downgrades are scheduled for the end of the billing period. If claimed resources exceed the new capacity, the excess claims are marked as "interim" with an `expiredAt` timestamp set to the billing period end. Customers retain full access until then, and claims automatically expire when the downgrade takes effect.

- **`timing: 'immediately'`:** The adjustment is blocked if claimed resources exceed the new capacity. You'll receive an error message:
...
The customer must first release enough resources (via `releaseResource`) before an immediate downgrade can proceed.
...
With the default `auto` timing, downgrades are scheduled for the end of the billing period. Flowglad automatically manages excess claims for you:

```typescript lib/downgrade-with-seats.ts
import { flowglad } from './flowglad'

export async function downgradeToStarterPlan(customerExternalId: string) {
  const server = flowglad(customerExternalId)

  // Downgrade from Pro (25 seats) to Starter (5 seats)
  const result = await server.adjustSubscription({
    priceSlug: 'starter_monthly',
    // timing defaults to 'auto' - downgrades apply at end of billing period
  })

  // If current claims exceed the new capacity:
  // - The downgrade is scheduled for end of period
  // - Excess claims are marked with expiredAt timestamp
  // - Claims automatically expire when the downgrade takes effect
  // - Customer retains full access until the billing period ends
  return result.subscription
}
```

<Note>
When a scheduled downgrade would reduce capacity below current claims, excess claims are marked as "interim" with an `expiredAt` timestamp. They remain active until the billing period ends, then automatically expire. This gives customers time to decide which resources to keep.
</Note>

<Warning>
**Immediate downgrades** behave differently. If you use `timing: 'immediately'` and claimed resources exceed the new capacity, the adjustment is blocked. You must first release excess claims using `releaseResource` before an immediate downgrade can proceed.
</Warning>

### Building a Seat Adjustment UI

Here's how to build a UI that informs users about capacity changes during plan adjustments:
...
export async function previewPlanChange(params: {
  ...
  const excessSeats = Math.max(0, usage.claimed - params.newSeatCapacity)

  // Identify which members will lose access at end of billing period.
  // Customize this logic based on your needs (e.g., let admins choose,
  // exclude certain roles, use last-in-first-out, etc.)
  const affectedMembers = excessSeats > 0
    ? claims
        .filter(c => c.externalId !== null)
        .slice(0, excessSeats)
        .map(c => ({
  ...
  return {
    canDowngrade: true, // Downgrades are always allowed with auto timing
    currentSeats: usage.claimed,
    newCapacity: params.newSeatCapacity,
    excessSeats,
    affectedMembers
  }
}
