...
**On Cancellation:** When a subscription is canceled, all associated resource claims are automatically released with `releaseReason: "subscription_canceled"`. This ensures clean capacity management without manual cleanup.
...
Upgrading to a plan with more resource capacity is straightforward. Existing claims are preserved, and the new capacity is immediately available:

```typescript lib/upgrade-with-seats.ts
import { flowglad } from './flowglad'
...
export async function downgradeToStarterPlan(params: {
  ...
  const { usage } = await server.getResourceUsage({ resourceSlug: 'seats' })
  const newCapacity = 5  // Starter plan capacity

  if (usage.claimed > newCapacity) {
    // Step 2: Release excess claims before downgrading
  ...
  // Step 3: Now the downgrade will succeed
  const result = await server.adjustSubscription({
...
export async function validatePlanChange(params: {
  ...
  if (usage.claimed > params.newSeatCapacity) {
    ...
    // Return info about which members would need to be removed
...
All resource operations are available via REST API. See the [API Reference](/api-reference/introduction) for full details.

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/v1/resource-claims/{subscriptionId}/claim` | POST | Claim resources |
| `/api/v1/resource-claims/{subscriptionId}/release` | POST | Release resources |
| `/api/v1/resource-claims/{subscriptionId}/usage` | GET | Get single resource usage |
| `/api/v1/resource-claims/{subscriptionId}/usages` | GET | List all resource usages |
| `/api/v1/resource-claims/{subscriptionId}/claims` | GET | List active claims |

### Claim Resource

```bash
curl -X POST https://api.flowglad.com/api/v1/resource-claims/{subscriptionId}/claim \
...
```bash
curl -X POST https://api.flowglad.com/api/v1/resource-claims/{subscriptionId}/release \
  -H "Authorization: Bearer your-api-key" \
  -H "Content-Type: application/json" \
  -d '{
    "resourceSlug": "seats",
...
```bash
curl https://api.flowglad.com/api/v1/resource-claims/{subscriptionId}/usage?resourceSlug=seats \
  -H "Authorization: Bearer your-api-key"
```
...
```bash
curl https://api.flowglad.com/api/v1/resource-claims/{subscriptionId}/usages \
  -H "Authorization: Bearer your-api-key"
```
...
```bash
curl https://api.flowglad.com/api/v1/resource-claims/{subscriptionId}/claims?resourceSlug=seats \
  -H "Authorization: Bearer your-api-key"
```
