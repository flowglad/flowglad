...
**On Downgrade:** When adjusting a subscription to a plan with lower capacity, Flowglad's behavior depends on the `timing` parameter:

- **With `timing: 'auto'` (the default):** If the current claimed count exceeds the new capacity, the downgrade is scheduled for the end of the billing period. Customers retain their current capacity until then. During this interim, claims that exceed the *future* capacity are marked as temporary—they have an `expiredAt` timestamp set to the billing period end and automatically expire when the downgrade takes effect. This gives customers time to release excess claims before the change applies.

- **With `timing: 'immediately'`:** The adjustment is blocked if current claims exceed the new capacity. The customer must first release enough resources (e.g., via `releaseResource`) before the downgrade can proceed. The error message indicates what action is needed:
