...
app.post('/webhooks/flowglad', express.raw({ type: 'application/json' }), async (req, res) => {
  const signature = req.headers['flowglad-signature'] as string
  
  try {
    // Verify webhook signature
    const event = flowgladServer.verifyWebhook(req.body, signature)
