...
A Next.js app with authentication and billing interation, demonstrating a pricing model with subscription, single-payment, and usage.

**Location:** [examples/generation-based-subscription](https://github.com/flowglad/flowglad/tree/main/examples/generation-based-subscription)
...
export function requireFeature(featureSlug: string) {
  return async (req, res, next) => {
    try {
      ...
      const billing = await getFlowgladServer(userId).getBilling()
      const hasAccess = billing.checkFeatureAccess(featureSlug)
