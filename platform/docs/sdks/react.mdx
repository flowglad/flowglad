...
function SubscriptionManager() {
  const { uncancelSubscription, currentSubscriptions } = useBilling()
  ...
  return (
    <div>
      <p>Your subscription is scheduled to cancel on {subscription.cancelScheduledAt}</p>
...
- Operation is idempotent. If the subscription is not scheduled to cancel, the method will silently succeed without uncancelling
