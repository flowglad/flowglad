...
When adjusting subscriptions that include [resources](/features/resources) like seats or API keys, Flowglad handles capacity changes automatically:

- **Upgrades:** Claims are preserved, and new capacity is immediately available
- **Scheduled Downgrades (default):** Claims exceeding the new capacity are marked as "interim" with an `expiredAt` timestamp. They remain active until the billing period ends, then automatically expire.
- **Immediate Downgrades:** If you force an immediate downgrade with `timing: 'immediately'`, the adjustment is blocked if claimed resources exceed the new capacity.
...
export async function downgradeToStarterPlan(customerExternalId: string) {
  ...
  // With auto timing, excess claims are automatically scheduled to expire
