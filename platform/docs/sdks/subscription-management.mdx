...
**Behavior:**
- The operation is idempotent - calling it on a subscription that isn't scheduled for cancellation will silently succeed without uncancelling
    ...
    return (
      <>
        <button onClick={handleUncancel} disabled={isLoading}>
          {isLoading ? 'Uncancelling...' : 'Keep My Subscription'}
        </button>
        {error && <p className="text-sm text-destructive">{error}</p>}
      </>
    )
