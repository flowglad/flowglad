...
- **`getResourceUsages`** (server): Returns all resources and their usage stats for a subscription
- **`getResourceUsage`** (server): Returns usage for a single resource by slug or ID
- **`claimResource`** (server): Reserves capacity. Use `quantity` for anonymous claims, `externalId` for named claims
- **`releaseResource`** (server): Frees capacity. Use `quantity` for FIFO release, `externalId` for named release
- **`listResourceClaims`** (server): Returns all active claims, optionally filtered by resource
...
export async function addTeamMember(
  ...
  const server = flowglad(orgExternalId)
...
export async function reserveConnections(
...
export async function removeTeamMember(orgExternalId: string, userId: string) {
  const server = flowglad(orgExternalId)
...
Release anonymous claims by quantity (FIFO—oldest first):
...
Get all active (unreleased) claims for a resource:

```ts lib/team-management.ts
import { flowglad } from '@/utils/flowglad'

export async function getTeamMembers(orgExternalId: string) {
  const server = flowglad(orgExternalId)
...
Reserve resource capacity. Choose ONE mode:
