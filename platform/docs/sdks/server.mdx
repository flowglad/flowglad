...
- If the subscription is not scheduled to cancel, the method will silently succeed without uncancelling (idempotent)
- If the subscription is in a terminal state (e.g., `canceled`), the method will silently succeed without uncancelling
