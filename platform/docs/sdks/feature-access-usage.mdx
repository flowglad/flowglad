...
- For `createUsageEvent` (server only): provide `{amount: number, subscriptionId: string, transactionId: string, properties?: Record<string, unknown>, usageDate?: number}` along with exactly one of:
  ...
  The optional `properties` field is for usage meter of count distinct properties aggregation type. See [here](/api-reference/usage-events/create-usage-event#body-usage-event) for more details on the parameters.
