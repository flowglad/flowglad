...
For Next.js applications using Better Auth, you may want a client-side wrapper component that calls `authClient.useSession()` to keep session state reactive for client components. FlowgladProvider no longer takes a `loadBilling` prop, so you can mount it directly if you don't need session-aware rendering.
    ...
    <Callout type="warn">
  For Better Auth apps in Next.js, use a client-side wrapper component that calls `authClient.useSession()` if you need session-driven client UI to re-render as auth changes.
    </Callout>
