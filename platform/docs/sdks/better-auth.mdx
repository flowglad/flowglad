...
* **No Separate Route Handler**: All Flowglad endpoints are mounted directly on your Better Auth API
...
2. **Billing API Endpoints**: The plugin exposes all Flowglad billing endpoints directly on your Better Auth API at `{basePath}/flowglad/*`. No separate route handler needed.
