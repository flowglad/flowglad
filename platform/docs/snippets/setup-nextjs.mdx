...
// For Pages Router, you'll need to extract customerExternalId from the request
// and use it to create a scoped FlowgladServer instance
export default createPagesRouterRouteHandler(async (req) => {
  const userId = await getUserIdFromRequest(req)
