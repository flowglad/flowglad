...
7. **Existing usage events with `priceId: null` are backfilled** to use the no_charge price for their meter
8. **`usageEvent.priceId` is NOT NULL** - enforced at the schema level after backfill completes
...
- [ ] **Backfill script updates existing usage events with `priceId: null` to use the no_charge price**
- [ ] **`usageEvent.priceId` is NOT NULL after migration (enforced at schema level)**
- [ ] **Billing calculations correctly treat no_charge price events as $0 contribution**
...
3. **Existing priceless usage events ARE backfilled.** We backfill existing events with `priceId: null` to use the no_charge price for their meter. This enables making `priceId` NOT NULL and simplifies billing code (no null checks needed). Since no_charge prices are $0, this doesn't change any billing amounts—it just fills in the reference.
4. **usageEvent.priceId becomes NOT NULL.** After backfill completes, we run a migration to make the column non-nullable. This enforces data integrity at the schema level.
...
6. **Existing priceless events ARE backfilled**: Existing usage events with `priceId: null` are backfilled to use the no_charge price for their meter. Since no_charge is always $0, this doesn't change billing amounts—it just ensures referential integrity. This enables making `priceId` NOT NULL.

7. **usageEvent.priceId becomes NOT NULL**: After the backfill, we make the column non-nullable. This enforces at the schema level that all usage events have a price, eliminating null-handling code paths in billing logic.

8. **Backfill is safe**: Creating no_charge prices for existing meters and backfilling events doesn't affect existing subscriptions or billing. It just provides consistent data and enables schema constraints.

9. **No_charge in pricing.yaml is rejected**: Users cannot specify `_no_charge` usage prices in `pricing.yaml`. The validation in `setupSchemas.ts` rejects any usage price with a slug ending in `_no_charge`. These prices are auto-generated and managed by the system.
...
describe('usage events use default price', () => {
  ...
  it('bulk insert correctly resolves a mix of default and explicit prices', async () => {
    // setup: usage meter with default price D and non-default price P
    // action: bulk insert events where some specify priceId=P and others use usageMeterSlug
    // expect:
    // - events with explicit priceId=P have priceId=P
    // - events with usageMeterSlug have priceId=D (the default)
    // - all events have correct usageMeterId
  })
...
describe('billing calculation with no_charge prices', () => {
  it('calculates $0 contribution for usage events with no_charge price', async () => {
    // setup:
    // - usage meter with no_charge price (unitPrice: 0)
    // - create several usage events with the no_charge price
    // - run billing calculation for the period
    // expect:
    // - total charge amount from these events equals $0
    // - events are included in the billing run (not skipped)
    // - invoice line item shows quantity but $0 amount
  })

  it('correctly calculates mixed billing with charged and no_charge events', async () => {
    // setup:
    // - usage meter with both a $10 price and no_charge price
    // - create events: 5 with $10 price, 3 with no_charge price
    // - run billing calculation
    // expect:
    // - total equals 5 * $10 = $50 (no_charge events contribute $0)
    // - both prices appear in billing breakdown
  })
})
```
...
### Patch 5: Backfill No Charge Prices and Usage Events

**Files to create:**
- `src/scripts/backfillNoChargePricesAndUsageEvents.ts`
...
4. **Backfill existing usage events with `priceId: null`** to use the no_charge price for their meter

```ts
export const backfillNoChargePricesAndUsageEvents = async () => {
  const allUsageMeters = await selectAllUsageMeters()

  for (const meter of allUsageMeters) {
    await adminTransaction(async ({ transaction }) => {
      // Step 1: Ensure no_charge price exists
      const noChargeSlug = getNoChargeSlugForMeter(meter.slug)
      let noChargePrice = await selectPriceBySlugAndPricingModelId(
        noChargeSlug,
        meter.pricingModelId,
        transaction
      )
      ...
      if (!noChargePrice) {
        ...
        noChargePrice = await insertPrice(noChargePriceInsert, transaction)
      }

      // Step 2: Backfill usage events with null priceId
      await transaction.db
        .update(usageEvents)
...
describe('backfill usage events with null priceId', () => {
  it('updates usage events with null priceId to use the no_charge price', async () => {
    // setup:
    // - usage meter with no_charge price
    // - 5 usage events with priceId: null for that meter
    // action: run backfill
    // expect:
    // - all 5 events now have priceId set to the no_charge price id
    // - usageMeterId unchanged
  })

  it('does not modify usage events that already have a priceId', async () => {
    // setup:
    // - usage meter with no_charge price and regular price P
    // - 3 events with priceId: P, 2 events with priceId: null
    // action: run backfill
    // expect:
    // - 3 events still have priceId: P (unchanged)
    // - 2 events now have priceId: no_charge price id
  })

  it('backfills events for all meters in a large dataset', async () => {
    // setup:
    // - 10 meters, each with some null priceId events
    // action: run backfill
    // expect: all events across all meters have priceId set
  })
})
```
...
### Patch 7: Make usageEvent.priceId NOT NULL

After the backfill in Patch 5 ensures all existing usage events have a `priceId`, we can make the column non-nullable.

**Files to modify:**
- `src/db/schema/usageEvents.ts`

**Changes:**
1. Change `priceId` column from nullable to NOT NULL
2. Run `bun run migrations:generate` to create the migration
...
// After:
priceId: text('price_id').references(() => prices.id).notNull(),
```

**Migration safety:**
- This migration must run AFTER Patch 5 backfill completes
- The backfill ensures no `priceId: null` rows exist
- The migration will fail if any null values remain (which is the intended safety check)

**Test cases:**
```ts
describe('usageEvent.priceId NOT NULL constraint', () => {
  it('rejects inserting usage event without priceId', async () => {
    // setup: attempt to insert usage event with priceId: null
    // expect: database constraint violation error
  })

  it('requires priceId in usage event insert schema', async () => {
    // setup: validate usage event insert with missing priceId against Zod schema
    // expect: Zod validation error for required field
  })

  it('allows inserting usage event with valid priceId', async () => {
    // setup: insert usage event with valid priceId
    // expect: success, event created
  })
})
```
...
- Patch 5 (backfill prices & usage events) depends on Patches 2 and 3 to use the same helpers
- Patch 6 (UI changes) depends on Patches 2 and 3 because it needs no_charge detection and cascade logic
- **Patch 7 (priceId NOT NULL) depends on Patch 5 - must run AFTER backfill ensures no null priceIds remain**
...
- Then Patches 1-7 from this gameplan can be executed
- This replaces PR 8 and PR 9 from the existing gameplan (auto-create and backfill)
- Patch 7 makes `usageEvent.priceId` NOT NULL, which should be the final migration after all backfills complete
