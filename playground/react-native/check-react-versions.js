...
function findReactVersions(dir, depth = 0, maxDepth = 5) {
  ...
  try {
    ...
    if (fs.existsSync(bunPath)) {
      ...
      for (const entry of entries) {
        if (entry.startsWith('react@')) {
          const reactPackagePath = path.join(bunPath, entry, 'node_modules', 'react', 'package.json');
          if (fs.existsSync(reactPackagePath)) {
            try {
              const packageJson = JSON.parse(fs.readFileSync(reactPackagePath, 'utf8'));
              const version = packageJson.version;
              const fullPath = path.resolve(path.join(bunPath, entry, 'node_modules', 'react'));
    ...
    if (parentDir !== dir) {
      const parentVersions = findReactVersions(parentDir, depth + 1, maxDepth);
...
if (versions.size === 1) {
  ...
  for (const [version, paths] of versions.entries()) {
    console.log(`📦 React ${version} (found in ${paths.length} location(s)):`);
    paths.forEach((p, i) => {
      console.log(`   ${i + 1}. ${p}`);
    });
    console.log('');
  ...
  console.log('❌ Multiple React versions detected! This can cause "Invalid hook call" errors.');
  console.log('\n💡 Solutions:');
  console.log('   1. Ensure all packages use the same React version');
  console.log('   2. Use yarn/npm resolutions or bun overrides to force a single version');
  console.log('   3. Check if any dependencies have peer dependency conflicts');
  console.log('   4. Clear node_modules and reinstall: bun install --force\n');
