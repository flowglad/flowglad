...
// Create mock billing data with customer
...
describe('useCustomerDetails', () => {
  ...
  it('uses billingMocks in dev mode', async () => {
    ...
    // Should have customer from billingMocks
  ...
  it('throws error in dev mode when billingMocks.customer missing', () => {
    // Create billing mocks without customer
    const billingMocksWithoutCustomer = {
      ...createMockBillingData(),
      customer: undefined,
    }

    expect(() => {
      renderHook(() => useCustomerDetails(), {
        wrapper: createWrapper(true, billingMocksWithoutCustomer),
      })
    }).toThrow(
